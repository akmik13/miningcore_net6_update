    ...
    public static Task Main(string[] args)
    {
        try
        {
            ...
            var hostBuilder = new HostBuilder()
                .ConfigureServices((ctx, services) =>
                {
                    ...
                    services.AddHostedService<PayoutSchemeCoordinator>();
                    services.AddHostedService<ShareCharger>();
                    services.AddHostedService<RewardRecalculator>();
                    services.AddHostedService<MetricsPublisher>();

                    if (clusterConfig?.Notifications?.NotificationProcessing?.Enabled == true)
        ...
        catch (AggregateException ex)
        {
            if (ex.InnerExceptions.Count > 0)
            {
                ...
                if (innerEx is PoolStartupException)
                {
...
");
    ...
    private static bool PreFlightChecks()
    {
        // root check
        ...
        {
            Console.WriteLine("Miningcore requires 64-Bit Windows");
            return false;
        }
